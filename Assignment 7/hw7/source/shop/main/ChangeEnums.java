...
enum ChangeEnums {
    ...
    private static NewInterface formControl;
    private static UIFormTest testInts;

    private static FactoryOverride SuperUI = new FactoryOverride();

    ChangeEnums(UIMenuAction r){ 
        this.run = r;
    }
    ...
    static class INIT{
        public INIT(UI ui, Inventory inventory, NewInterface getVideoForm, UIFormTest nt) { // initialize constructors
            _ui = ui;
            _inventory = inventory;
            formControl = getVideoForm;
            testInts = nt;
    ...
    static class addremove implements UIMenuAction{
        public void run(){
            String[] result1 = _ui.processForm(formControl);
            ...
            f.add("Number of copies to add/remove", testInts);
